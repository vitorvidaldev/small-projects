package dev.vitorvidal.highscore
...
open class HighScore : Game() {
    ...
    private val assetsManager = object ? AssetManager() {
        override fun <T : Any?> get(fileName: String?, type: Class<T>?): T {
            return try {
                ...
                ktx.log.error(ex) {
                    "Asset $fileName not yet loaded! You should load it before accessing it! " + " Will force the loading of it. (Bad performance expected)"
                }
    ...
    override fun create() {
        ...
        assetsManager.setLoader(TileMap::class.java, TmxMapLoader(InternalFileHandleResolver()))
    ...
    companion object {
        ...
        private lateinit var game: HighScore
